...
function toDto(row) {
  ...
  return {
    ...
    preferredTimeWindow: row.preferred_time_window && Object.keys(row.preferred_time_window).length > 0 ? row.preferred_time_window : null,
    ...
    metadata: row.metadata && Object.keys(row.metadata).length > 0 ? row.metadata : null,
