...
router.post('/', express.json(), (req, res) => {
  try {
    ...
    logger.info('[voice-reply-content] Request from ManyChat. subscriber_id:', subscriberId, 'body keys:', Object.keys(req.body || {}));

    if (!subscriberId) {
      logger.warn('[voice-reply-content] No subscriber_id in request');
      return res.json({ version: 'v2', content: { type: 'instagram', messages: [{ type: 'text', text: '' }] } });
    }

    const audioUrl = voiceReplyStore.get(subscriberId);
    logger.info('[voice-reply-content] Pending audio for subscriber', subscriberId, ':', audioUrl ? 'FOUND' : 'NOT FOUND');
    ...
    logger.info('[voice-reply-content] Returning audio URL:', audioUrl);
    res.json({
      version: 'v2',
      content: {
        type: 'instagram',
    ...
    logger.error('[voice-reply-content] Error:', err.message);
