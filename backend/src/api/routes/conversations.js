...
router.post('/:conversationId/send-voice', async (req, res) => {
  try {
    ...
    // Send audio to Instagram via ManyChat
    const manychatApiKey = decrypt(lead.manychat_api_key);
    const { sendManyChatFile, sendFlow } = require('../../services/manychatService');
    try {
      await sendManyChatFile(lead.external_id, audioPublicUrl, manychatApiKey);
      logger.info({ subscriberId: lead.external_id }, '[conversations/send-voice] Voice sent via sendContent');
    } catch (mcErr) {
      logger.warn({ status: mcErr.response?.status, err: mcErr.message }, '[conversations/send-voice] sendContent failed, trying flow');
      ...
      if (flowNs) {
        ...
        logger.info('[conversations/send-voice] Voice sent via flow fallback');
      } else {
        throw mcErr;
      }
    }
